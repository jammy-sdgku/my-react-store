...
const filtered = this.products.filter(product => product.category
=== category);
resolve(filtered);
}
}, delay);
...
return new Promise((resolve) => {
setTimeout(() => {
const onSale = this.products.filter(product =>
...
this.products.length,
averageRating: this.products.reduce((sum, p) => sum + p.rating, 0) /
this.products.length,
